import { Component, Input, OnChanges, OnInit, SimpleChanges } from '@angular/core';
import { IProducts } from '../../modals/Iproducts';
import { Icategory } from '../../modals/Icategory';

@Component({
  selector: 'app-product',
  templateUrl: './product.component.html',
  styleUrls: ['./product.component.scss']
})
export class ProductComponent implements OnInit, OnChanges {

  // InterFace Category
  // category: Icategory[];
  // Oringnal Products
  originalListProduct: IProducts[] = []
  // Clone Products 
  cloneListProduct: IProducts[] = []
  // Select Value With Ng Model From HTML Element Select (Two Way Binding)
  // returnValueWithNgmodel: number = 0;
  @Input() sendValueFromParent:number=0

  // Start constructor 
  constructor() { 
    // Create Category Useing Element Selector 
    // this.category = [
    //   { id: 0, name: "All Product"},  // What
    //   { id: 1, name: 'Mobile' },
    //   { id: 2, name: 'Laptops' },
    //   { id: 3, name: 'Display Screen' }
    // ]

    // Create Products Using Show In User 
    this.originalListProduct = [
      { id: 10, category: 1, name: "Iphone 13 pro max", Quantity: 0, Price: 1000, image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ4ZTqUHLNkwvGjsLThyiD_MRVF8D_UIesKNikqB0ZyxgoQL-RZ3optW4FmiH3jXIjQqDY&usqp=CAU" },
      { id: 5, category: 1, name: "samsung s21 ultra", Quantity: 3, Price: 9000, image: "https://media.btech.com/media/catalog/product/cache/8e279240a6612e9579d3be696bac1c07/s/a/samsung-galaxy-note-20-ultra-black_1.jpg" },
      { id: 7, category: 1, name: "oppo find x3 pro", Quantity: 4, Price: 8000, image: "https://3edda.com/system/mobile_images/images/000/000/765/original/0010241_oppo-find-x-256gb-dual-sim-bordeaux-red-glacier-blue_610.jpg?1604429317" },
      { id: 8, category: 2, name: "laptop Samsung", Quantity: 5, Price: 8000, image: "https://cdn1.smartprix.com/rx-iR50BHW78-w1200-h1200/R50BHW78.jpg" },
      { id: 8, category: 2, name: "laptop mackbook", Quantity: 2, Price: 1500, image: "https://www.digitaltrends.com/wp-content/uploads/2021/11/macbook-pro-2021-01.jpg?resize=625%2C417&p=1" },
      { id: 8, category: 2, name: "laptop Dell", Quantity: 1, Price: 9000, image: "https://www.lifewire.com/thmb/_Z0uM37TmAQBHLcdvpLMsEn96JY=/1500x1001/filters:fill(auto,1)/4136227-4-5c208c9146e0fb00017d21e7.jpg" },
      { id: 8, category: 3, name: "screen samsung", Quantity: 1, Price: 8000, image: "https://www.dominodisplay.com/media/catalog/product/cache/99b4ecd9c4d5eebe05ee34361d991775/i/m/img_qpa8k_1_1new_1.jpg" },
      { id: 8, category: 3, name: "screen Iphone", Quantity: 2, Price: 8000, image: "data:image/jpeg;base64," },
      { id: 8, category: 3, name: "screen lenovo", Quantity: 0, Price: 8000, image: "https://www.lenovo.com/medias/61F6MAT2UK%20Listing%20Image.png?context=bWFzdGVyfGltYWdlc3wxMjM3MzN8aW1hZ2UvcG5nfGltYWdlcy9oM2YvaGU4LzEzMDIzMTU3NzgwNTEwLnBuZ3xmYmQxNjJhMWVhN2Y0Nzk1MjE5M2ExZGZlYzFjYzY3OGYzNjQ2NzNiYWY2N2RlMWM4YjIyZGVmMDQzMjZhNmE0" },
      { id: 8, category: 3, name: "screen hp", Quantity: 1, Price: 8000, image: "https://www.lenovo.com/medias/61F6MAT2UK%20Listing%20Image.png?context=bWFzdGVyfGltYWdlc3wxMjM3MzN8aW1hZ2UvcG5nfGltYWdlcy9oM2YvaGU4LzEzMDIzMTU3NzgwNTEwLnBuZ3xmYmQxNjJhMWVhN2Y0Nzk1MjE5M2ExZGZlYzFjYzY3OGYzNjQ2NzNiYWY2N2RlMWM4YjIyZGVmMDQzMjZhNmE0" },

    ]
   


    // Copy From Original Product To Clone ( Useing Show All Products )
    this.cloneListProduct = this.originalListProduct;

  } // End constructor
  ngOnChanges(): void {
    
    this.showWithFilter()

  }



  // This Is Function Or Interface Ng On Init  
  ngOnInit(): void {
  }
  
  // Function Decrease With Button Buy
  dec(item: any) {

    if (item.Quantity > 1) {

      item.Quantity -= 1

    } else {

      item.Quantity = 0
    }

  }

  // Function Filtrs What Do You Want To Show 
  // showWithFilter() {

  //   this.cloneListProduct = this.originalListProduct.filter(pro => pro.category == this.returnValueWithNgmodel);

  // }

  showWithFilter() {

    this.cloneListProduct = this.originalListProduct.filter(pro => pro.category == this.sendValueFromParent);

  }
  
} // End The Class 
